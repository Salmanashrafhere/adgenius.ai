...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (userData) {
        ...
        // Fetch campaigns from API - optimized for dashboard
        const fetchCampaigns = async () => {
          try {
            // Only fetch latest 5 campaigns, and use lightweight full=false mode
            const response = await fetch(`/api/campaigns?userId=${parsedUser.id}&limit=5&full=false`);
            const data = await response.json();
            if (response.ok && data.success) {
              setCampaigns(data.campaigns);
              setTotalCount(data.totalCount || 0);
              setTotalAdsCount(data.totalAdsCount || 0);
            } else {
              ...
              if (savedCampaigns) {
                ...
                setTotalAdsCount(parsed.reduce((sum, c) => sum + (c.adsCount || 0), 0));
              }
            }
          } catch (err) {
  ...
  ], [totalCount, totalAdsCount, user?.credits]);
