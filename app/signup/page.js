...
function passwordStrength(password) {
  ...
  if (score <= 1) return { score: 1, label: "Weak", segments: 1 };
  if (score === 2) return { score: 2, label: "Fair", segments: 2 };
  if (score === 3) return { score: 3, label: "Good", segments: 3 };
...
export default function SignupPage() {
  ...
  async function handleSubmit(e) {
    ...
    const next = {
      ...
      terms: terms ? "" : "You must agree to the terms to continue",
    };
    setErrors(next);
  ...
  const strengthText =
    strength.segments <= 1 ? "text-red-600" : strength.segments === 2 ? "text-amber-600" : strength.segments === 3 ? "text-indigo-600" : "text-emerald-600";
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div>
              ...
              <div className="relative">
                <User className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" aria-hidden />
                <input
                  id="signup-name"
                  name="fullName"
                  type="text"
                  ...
                  className={`w-full rounded-lg border bg-white py-2.5 pl-10 pr-3 text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 ${
                    errors.fullName ? "border-red-300 focus:border-red-500" : "border-slate-200 focus:border-indigo-600"
                  }`}
                ...
                <div className="mt-2 space-y-1.5">
                  <div className="flex gap-1" aria-hidden>
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        key={i}
                        className={`h-1.5 flex-1 rounded-full transition-colors ${i <= strength.segments ? strengthColor : "bg-slate-200"}`}
                      />
                    ))}
                  </div>
                  <p className={`text-xs font-medium ${strengthText}`}>{strength.label}</p>
                </div>
              ) : null}
              {errors.password ? (
                <p className="mt-1.5 text-sm text-red-600" role="alert">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full rounded-lg border bg-white py-2.5 pl-10 pr-11 text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 ${
                    errors.confirmPassword ? "border-red-300 focus:border-red-500" : "border-slate-200 focus:border-indigo-600"
                  }`}
                  placeholder="••••••••"
